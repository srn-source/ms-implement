...
class BaseProcessor:
    ...
    def kate_process(self):
        ...
        logging.info(
            "n_dev = {} n_train = {} all corpus = {}".format(
                n_dev, n_train, len(corpus)
            )
        )
        ...
        if self.kate_metric == "euclidean":
            ...
            nbrs = NearestNeighbors(
                n_neighbors=self.k, algorithm="ball_tree", n_jobs=-1
            ).fit(emb_train)
            distances, indices = nbrs.kneighbors(emb_dev)
            ...
            if self.reversed:
                values, indices = torch.topk(
                    -torch.from_numpy(dist_matrix), k=self.k, dim=-1
                )
                values_t, indices_t = torch.topk(
                    torch.from_numpy(dist_matrix), k=self.k, dim=-1
                )
                # print("values == ",values)
    ...
    def make_label_ids(self, tokenizer):
        ...
        for label, label_encoded in zip(
            self.labels,
            tokenizer.batch_encode_plus([sp + l for l in self.labels])["input_ids"],
        ):
    ...
    def initialize_all(self):
        ...
        if self.model_name in model1.keys():
            ...
            self.model_name in model2.keys()
        ):  # please remind that tokenize dont have ""
    ...
    def probe(self, prompt, model, tokenizer, model_type, gen_leng):
        ...
        if model_type in ["model1", "model2"]:
            ...
            # self.make_label_ids(tokenizer)
            # print("prompt = ",prompt)
            batch = tokenizer.encode_plus(prompt, return_tensors="pt", padding=True)
            ...
            decoded = tokenizer.batch_decode(
                encoded[:, input_length:], skip_special_tokens=True
            )

            hidd = output.hidden_states
            # print("prompt = ",prompt)
            # print("decoded = ",decoded)
            # print("output.hidd len = ",len(hidd))
            ...
            if gen_leng == 1:
                ...
                for i, raw_completion in enumerate(decoded):
                    ...
                    # perplexity = self.perplexity(model,last_layer_filtered,input_ids_filtered)
    ...
    def globalentropy_ordering(self, random_train_ids):
        ...
        for perm in tqdm(permutations(random_train_ids), desc="Subsets", leave=False):
            ...
            for data_test in probe_examples:
                ...
                label_ans, pred = self.probe(
                    prompts123, model, tokenizer, model_type, 1
                )
    ...
    def generate_datasets(self, seed: int):
        ...
        random_test_ids = deterministic_random(42).sample(
            range(len(self.test_split)), k=1000
        )
        ...
        random_train_ids = deterministic_random(self.seed).sample(
            range(len(self.train_split)), k=self.k
        )
